...
PHP_INI_BEGIN()
        ...
        STD_PHP_INI_ENTRY
        ("yac.keys_memory_size", "4M", PHP_INI_SYSTEM, OnChangeKeysMemoryLimit, k_msize, zend_yac_globals, yac_globals)
        STD_PHP_INI_ENTRY
        ("yac.values_memory_size", "64M", PHP_INI_SYSTEM, OnChangeValsMemoryLimit, v_msize, zend_yac_globals,
         yac_globals)
        STD_PHP_INI_ENTRY("yac.compress_threshold", "-1", PHP_INI_SYSTEM, OnChangeCompressThreshold, compress_threshold,
                          zend_yac_globals, yac_globals)
        STD_PHP_INI_ENTRY
        ("yac.enable_cli", "0", PHP_INI_SYSTEM, OnUpdateBool, enable_cli, zend_yac_globals, yac_globals)
PHP_INI_END()
...
static int yac_add_impl(zend_string *prefix, zend_string *key, zval *value, int ttl, int add) /* {{{ */ {
    ...
    switch (Z_TYPE_P(value)) {
        ...
        case IS_FALSE:
            ...
            break;
        case IS_LONG:
            ret = yac_storage_update(ZSTR_VAL(key), ZSTR_LEN(key), (char *) &Z_LVAL_P(value), sizeof(long), flag, ttl,
                                     add, tv);
            break;
        case IS_DOUBLE:
            ret = yac_storage_update(ZSTR_VAL(key), ZSTR_LEN(key), (char *) &Z_DVAL_P(value), sizeof(double), flag, ttl,
                                     add, tv);
            break;
        case IS_STRING:
        case IS_CONSTANT: {
            ...
            // 可能考虑数据压缩
            // 关键指标:
            // 1. compress_threshold 或者1M就要启动压缩 fastlz_compress
            // 2. 最大长度不超过 64M
            if (Z_STRLEN_P(value) > YAC_G(compress_threshold) || Z_STRLEN_P(value) > YAC_STORAGE_MAX_ENTRY_LEN) {
                ...
                ret = yac_storage_update(ZSTR_VAL(key), ZSTR_LEN(key), Z_STRVAL_P(value), Z_STRLEN_P(value), flag, ttl,
                                         add, tv);
            }
        }
            break;
        ...
        case IS_OBJECT: {
...
#endif
            {
                if (buf.s->len > YAC_G(compress_threshold) || buf.s->len > YAC_STORAGE_MAX_ENTRY_LEN) {
                    ...
                    ret = yac_storage_update(ZSTR_VAL(key), ZSTR_LEN(key), compressed, compressed_len, flag, ttl, add,
                                             tv);
                    efree(compressed);
                } else {
                    ret = yac_storage_update(ZSTR_VAL(key), ZSTR_LEN(key), ZSTR_VAL(buf.s), ZSTR_LEN(buf.s), flag, ttl,
                                             add, tv);
                }
                smart_str_free(&buf);
            } else {
